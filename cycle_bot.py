...
# For now read from env, default 0
...
STAGE_NAMES = ["Hope","Optimism","Belief","Thrill","Euphoria","Complacency",
...
def safe(s):
    """Replace $ with dollar sign unicode to avoid matplotlib mathtext parser."""
...
def rbox(ax, x, y, w, h, r=0.008, fc=CARD, ec=BORDER, lw=0.5, z=1):
    ax.add_patch(FancyBboxPatch((x,y),w,h,
        ...
        zorder=z, transform=ax.transAxes, clip_on=False))
...
def bdg(ax, cx, cy, lbl, bg, fg, sz=6.5, z=6):
    lbl = str(lbl).replace('$', '$')
    ax.text(cx,cy,lbl,fontsize=sz,color=fg,ha="center",va="center",
            fontweight="bold",transform=ax.transAxes,zorder=z+1,
...
def build_dashboard(results, vol_sym, vol_val, vol_rname, vol_mult, ts):
    ...
    n   = len(results)
    W   = 6.5          # inches — narrow/portrait optimised
    # Each ticker card height + header + footer
    TICKER_H = 5.8     # inches per ticker card
    HEADER_H = 2.2
    FOOTER_H = 1.8
    ...
    M  = 0.038          # left/right margin
    W2 = 1 - 2*M        # usable width
    y  = 0.995          # cursor from top

    # ── HEADER ──────────────────────────────────────────────────
    txt(ax, M, y, "MARKET CYCLE DASHBOARD", sz=11, bold=True)
    txt(ax, 1-M, y, ts, sz=6.5, c=T3, ha="right")
    y -= 0.012
    ...
    vox = [
        (vol_sym,    vol_str,           vol_color(vol_rname)),
        ("Regime",   vol_rname,         vol_color(vol_rname)),
        ("Mult",     f"{vol_mult:.2f}×",T1),
        ("Capital",  f"£{int(CAPITAL_PER_TICKER)}",T1),
        ("Split",    "50% Core · 50%",  T2),
    ]
    xi = M + 0.012
    sw = W2 / len(vox)
    for lbl,val,col in vox:
        txt(ax, xi, y-sh*0.22, lbl, sz=6, c=T3)
        txt(ax, xi, y-sh*0.68, val, sz=7.5, c=col, bold=True)
        xi += sw
    y -= sh + 0.012

    # Cycle bar
    txt(ax, M, y, "Cycle position", sz=6, c=T3)
    y -= 0.010
    bh = header_frac * 0.28
    ...
    for r in results:
        ...
        # ── ROW 1: ticker + price + stage
        txt(ax, lx, row, r["symbol"], sz=13, bold=True)
        txt(ax, lx+0.14, row, f"USD{r['close']:.2f}", sz=11, bold=True, c=T1)
        txt(ax, rx, row, f"{r['pct_from_high']:.1f}% from 52W high", sz=6.5, c=T2, ha="right")
        row -= 0.020

        bdg(ax, lx+0.055, row, r["stage_name"], sc_bg+"66", sc_bg, sz=6.8)
        tbg = G_BG if r["above_200ma"] else R_BG
        tfg = G_FG if r["above_200ma"] else R_FG
        bdg(ax, lx+0.175, row, ("↑ Above" if r["above_200ma"] else "↓ Below")+" 200MA", tbg, tfg, sz=6.5)
        if r["near_trigger"]:
            bdg(ax, lx+0.295, row, "⚠ Near panic", A_BG, A_FG, sz=6.5)
        row -= 0.018

        hline(ax, lx, rx, row)
        row -= 0.010

        # ── ROW 2: RSI | MACD | 200MA | S/R
        ind = [("RSI(14)", str(r["rsi"])),
               ("MACD",    r["macd_text"]),
               ("200MA",   f"USD{r['ma200']:.2f}"),
               ("S / R",   f"USD{r['sup']} / ${r['res']}")]
        iw = W2 * 0.24
        ix = lx
        for lbl,val in ind:
            rbox(ax,ix,row-0.030,iw-0.008,0.033,r=0.005,fc=INNER,ec="none",lw=0)
            txt(ax,ix+0.005,row-0.008,lbl,sz=5.8,c=T3)
            txt(ax,ix+0.005,row-0.022,val,sz=6.5,bold=True)
            ix += W2*0.245
        row -= 0.044

        # ── ROW 3: 52W pullback levels
        rbox(ax,lx,row-0.018,W2-0.028,0.022,r=0.004,fc=INNER,ec="none",lw=0)
        txt(ax,lx+0.006,row-0.007,
        ...
        # ── CAPITAL STATUS BLOCK ─────────────────────────────────
        # Three clearly labelled columns: Already Deployed | Signal Today | Available
        txt(ax, lx, row, "CAPITAL POSITION", sz=6, c=T3, bold=True)
        row -= 0.016

        bw = (W2 - 0.028) / 3
        for i,(lbl,val,sub,bar_pct,bar_col) in enumerate([
            ("Already deployed",
             f"£{int(r['already_deployed'])}",
             f"Currently in position",
            ...
            ("Signal today",
             ...
             ORANGE),
            ("Still available",
             f"£{int(r['available']-(r['core_signal']+r['tactical_signal']))}",
             f"Undeployed capital",
             int(max(0,r['available']-r['core_signal']-r['tactical_signal'])/CAPITAL_PER_TICKER*100),
             B_FG),
        ]):
            bx = lx + i*(bw+0.006)
            rbox(ax,bx,row-0.058,bw,0.060,r=0.006,fc=INNER,ec="none",lw=0)
            txt(ax,bx+0.006,row-0.012,lbl,sz=5.8,c=T3)
            txt(ax,bx+0.006,row-0.027,val,sz=9.5,bold=True,c=T1)
            txt(ax,bx+0.006,row-0.039,sub,sz=5.5,c=T3)
            pbar_draw(ax,bx+0.006,row-0.052,bw-0.012,0.007,bar_pct,bar_col)
        row -= 0.070

        # Core and tactical action badges
        txt(ax, lx, row, "Core:", sz=6.5, c=T2)
        ca_bg = G_BG if "BUY" in r["core_action"] else A_BG
        ca_fg = G_FG if "BUY" in r["core_action"] else A_FG
        bdg(ax, lx+0.072, row, r["core_action"], ca_bg, ca_fg, sz=6.5)

        txt(ax, lx+0.22, row, "Tactical:", sz=6.5, c=T2)
        ta_bg = G_BG if r["tactical_action"]=="BUY" else B_BG
        ta_fg = G_FG if r["tactical_action"]=="BUY" else B_FG
        bdg(ax, lx+0.322, row, f"{r['tactical_action']} · {r['buy_speed']}", ta_bg, ta_fg, sz=6.5)
        row -= 0.018
        ...
        # ── PRICE ZONES ─────────────────────────────────────────
        txt(ax, lx, row, "PRICE ZONES", sz=6, c=T3, bold=True)
        row -= 0.016

        zones = [
            ("Entry zone",   f"USD{r['levels']['entry']}",                                       T1),
            ("Panic zone",   f"USD{r['levels']['panic_low']} – ${r['levels']['panic_high']}",    R_FG),
            ("Capitulation", f"USD{r['levels']['cap_low']} – ${r['levels']['cap_high']}",        R_FG),
            ("Target zone",  f"USD{r['levels']['target_low']} – ${r['levels']['target_high']}",  G_FG),
            ("Dist to panic",f"{r['levels']['dist_to_panic']:.1f}%",                           R_FG if r['levels']['dist_to_panic']>-20 else T2),
        ]
        # Two columns for zones to save vertical space
        ...
        for col_i, zone_slice in enumerate([zones[:mid], zones[mid:]]):
            ...
            for lbl,val,vc in zone_slice:
                rbox(ax,zx,zrow-0.020,W2*0.46,0.022,r=0.004,fc=INNER,ec="none",lw=0)
                txt(ax,zx+0.006,zrow-0.009,lbl,sz=6.2,c=T2)
                txt(ax,zx+W2*0.44,zrow-0.009,val,sz=6.5,c=vc,ha="right",bold=True)
                zrow -= 0.024
    ...
    txt(ax, M+0.014, fy, "VUAG — Reserve (profit proceeds only)", sz=7, c=T2, bold=True)
    fy -= 0.018
    ...
    for lbl,val,sub in vuag_data:
        rbox(ax,vxi,fy-0.052,vuag_bw-0.010,0.055,r=0.005,fc=INNER,ec="none",lw=0)
        txt(ax,vxi+0.005,fy-0.012,lbl,sz=5.8,c=T3)
        txt(ax,vxi+0.005,fy-0.027,val,sz=8.5,bold=True)
        if sub: txt(ax,vxi+0.005,fy-0.039,sub,sz=5.5,c=T3)
        pbar_draw(ax,vxi+0.005,fy-0.048,vuag_bw-0.020,0.006,80,TEAL)
        vxi += vuag_bw

    txt(ax, M+0.014, fy-0.060,
    ...
    # Summary — clearly separated three concepts
    txt(ax, M+0.014, fy, "PORTFOLIO SUMMARY", sz=6.5, c=T2, bold=True)
    fy -= 0.018
    ...
    for lbl,val,sub,col in s_items:
        txt(ax,sxi,fy,lbl,sz=6,c=T3)
        txt(ax,sxi,fy-0.014,val,sz=9,c=col,bold=True)
        txt(ax,sxi,fy-0.025,sub,sz=5.5,c=T3)
        sxi += sw2

    fy -= 0.036
    # Tactical triggers
    if triggers:
        bdg(ax, M+0.06, fy, f"🚨 Tactical BUY: {', '.join(triggers)}", R_BG, R_FG, sz=7)
    else:
        bdg(ax, M+0.08, fy, "No tactical triggers — market not in Panic/Capitulation", INNER, G_FG, sz=6.5)

    # Confidence dots
    dx = M+0.014
    dy = fy - 0.020
    for i in range(10):
        c2 = plt.Circle((dx+i*0.020, dy), 0.006,
                         color=TEAL if i<7 else "#333333",
                         transform=ax.transAxes, clip_on=False)
        ax.add_patch(c2)
    txt(ax, dx+10*0.020+0.008, dy, "7/10  confidence", sz=6.5, c=T3, va="center")
...
def send_photo(img_buf, caption):
    requests.post(
        ...
        data={"chat_id":TELEGRAM_CHAT_ID,"caption":caption[:900],"disable_web_page_preview":True},
        files={"photo":("dashboard.png",img_buf,"image/png")},
        timeout=60
    ).raise_for_status()
...
def main():
    ...
    if not results: raise RuntimeError("All symbols failed:\n"+"\n".join(failures))
    ...
    ]+[f"{r['symbol']}: {r['stage_name']} | Core {r['core_action']} | Tactical {r['tactical_action']}" for r in results]
    send_photo(img_buf,"\n".join(cap_lines))
    if failures: send_message("⚠️ Cycle bot failures:\n"+"\n".join(failures))
